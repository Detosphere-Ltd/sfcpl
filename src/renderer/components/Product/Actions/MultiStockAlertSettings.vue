<template>
	<div>
		<div class="uk-width-1-1 click" @click="election.length > 0 ? showModal = true : ''">
			<slot name="action-button">
				<button class="uk-button uk-button-primary" :disabled="selection.length < 1">
...
export default{
	...
	methods : {
		async updateStockAlerts(){
			this.loading = true
			await this.$axios.$patch(`/products/stock-alert-settings`, { products : [] , stock_alert_setting : this.stock_alert_settings})
			.then(()=>{
