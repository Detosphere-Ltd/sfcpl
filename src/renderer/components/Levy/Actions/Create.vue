<template>
	<div>
		...
		<div class="uk-width-1-1 uk-padding-remove"  uk-modal="esc-close: false; bg-close: false; sel-close: .levy-creation-modal; delay-hide: 0; delay-show: 0; stack: true" ref="modal" @beforeshow="$emit('opening')"  @beforehide="$emit('closing')">
			<div :class="colorMode" class="uk-modal-dialog transparent uk-padding-remove uk-width-large@s uk-margin-large-top uk-animation-slide-top-small uk-animation-fast uk-width-1-1 uk-border-rounded">
				...
				<div class="uk-width-1-1 background  uk-inline uk-flex uk-flex-column uk-border-rounded no-radius-sm bordered no-border-sm">
					<div class="uk-flex uk-flex-between uk-flex-middle tiny-padding border-bottom">
						...
						<a class="uk-button uk-button-small uk-button-danger uk-border-rounded levy-creation-modal">
							Close
						</a>
					</div>
					<div class="modal-content">
						...
						<div class="tiny-padding border-top">
							<button class="uk-button uk-button-primary uk-button-large uk-width-1-1" :class="{loading : loading}" :disabled="loading || !form.name || form.percent < 0 || !form.applies_to" @click.prevent="create">
								Create Levy
							</button>
						</div>
					</div>
...
<script>
import {base , create} from "@/mixins/levy"
export default {
