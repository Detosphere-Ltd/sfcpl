	...
	<div>
		<div class="uk-width-1-1 uk-flex uk-flex-top tiny-padding">
			<div class="square-30 uk-flex uk-flex-column uk-flex-center uk-flex-middle">
				<span class="mdi mdi-arrow-left-circle lni-large"></span>
			</div>

			<div class="uk-width-expand uk-margin-left uk-margin-right">
				<div class="uk-flex uk-flex-column">
					<h5 class="uk-margin-small-bottom uk-text-bold">
						Request Submitted to {{transfer.branch.name | branchize }} for {{transfer.total_units | pluralize('unit')}} of {{transfer.total_products | pluralize('product')}} 
					</h5>
				...
				<template v-if="can('change-transfer-request-status') && allowedStateChanges.includes(transfer.status)">
					<div class="uk-width-1-1 uk-margin-small-top tiny-padding uk-padding-remove-horizontal uk-padding-remove-bottom border-top uk-flex uk-flex-middle" v-show="showActions">
						...
						<request-actions-cancel :transfer="transfer" class="tiny-margin-right" @done="$emit('needs-to-refresh')" v-if="transfer.status === 'submitted'">
						</request-actions-cancel>
						<!-- <request-actions-raise-issue :transfer="transfer" class="tiny-margin-right" @done="$emit('needs-to-refresh')" v-if="transfer.status === 'dispatched' || transfer.status === 'resolved'"></request-actions-raise-issue>
							<request-actions-confirm :transfer="transfer" class="tiny-margin-right" @done="$emit('needs-to-refresh')" v-if="transfer.status === 'dispatched' || transfer.status === 'resolved'"></request-actions-confirm> -->
