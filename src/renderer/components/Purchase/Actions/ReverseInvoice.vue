				...
				<div class="uk-width-1-1 background bordered rounded raised-deep uk-animation-scale-up uk-animation-fast uk-width-large@s">
					...
					<template v-if="invoice.is_reversible">
						...
						<div class="tiny-padding border-top uk-flex uk-flex-center">
							<button class="uk-button uk-button-danger uk-margin-small-top" :disabled="!reason || reason.length < 5 || loading" @click="reverseInvoice">
					...
					<template v-else>
						<div class=" uk-overflow-auto modal-content">
							...
							<div class="tiny-padding uk-flex uk-flex-column">
								<h4>
									Sorry. You cannot reverse this purchase invoice!
								</h4>
								<template  v-if="invoice.irreversible_reasons && invoice.irreversible_reasons.length > 0">
									<p class="uk-margin-small-bottom">
										Reasons are provided below 
									</p>
									<ul>
										<li v-for="rs in invoice.irreversible_reasons" :key="rs">
											{{rs}}
										</li>
									</ul>
								</template>
...
<script>
import { bindProps , reversal } from "@/mixins/purchase/invoice"
export default{
