			...
			<div class="uk-width-1-1 uk-flex uk-flex-column stats-list" v-else>
				...
				<template v-if="stats && stats.length > 0">
					...
					<div class="stat-item tiny-padding uk-padding-remove-horizontal uk-flex uk-flex-center">
						<nuxt-link :to="{name: 'sales'}" class="uk-button uk-button-primary" v-if="can('view-sales')">
							View Sales Records
						</nuxt-link>
						<nuxt-link :to="{name: 'reports-performance'}" class="uk-margin-small-left uk-button uk-button-view" v-if="can('view-sales-statistics')">
...
<script>
import basic from "@/mixins/stats/sale_items"
export default{
	mixins : [basic],
	async mounted(){
		await this.getStats()
