	...
	<div class="background">
		<div class="tiny-padding border-bottom uk-flex uk-flex-between">
			<div class="uk-width-expand uk-flex uk-flex-middle">
				<tips-compare-products class="uk-margin-small-right"></tips-compare-products>
				<h4 class="uk-margin-remove">Compare Products</h4>
			</div>
			...
			<div class="uk-width-medium@m uk-inline">
				...
				<div class="uk-flex uk-flex-middle uk-width-auto uk-position-z-index uk-position-center-right" v-if="searchTerm || searching">
					<a class="square-20 background uk-border-circle bordered uk-flex uk-flex-column uk-flex-center uk-flex-middle click uk-margin-small-right" v-if="searchTerm && !searching" @click="getVariations(); searchTerm = ''">
		...
		<div class="tiny-padding border-bottom">
			<div class="uk-width-1-1 uk-flex uk-flex-middle uk-flex-between uk-flex-wrap">
				...
				<div class="uk-width-auto" v-if="productSelection && productSelection.length > 0">
					<div class="chip uk-background-danger pill uk-flex-middle click" @click="productSelection = []">
				...
				<div class="uk-width-auto uk-flex uk-flex-middle">
					<button class="uk-button uk-button-primary" :disabled="!productSelection || productSelection.length < 2 || !to || !from" @click="loadModal = true">
		...
		<section>
			<div class="uk-width-1-1">
				<div v-if="loadingData" class="uk-padding-small uk-flex uk-flex-center uk-flex-middle uk-flex-column">
				...
				<div v-else>
					...
					<div v-if="failed && errData"  class="uk-width-1-1 uk-flex uk-flex-column uk-flex-center uk-flex-middle background uk-padding uk-width-2xlarge@s  rounded uk-margin-auto uk-margin-top uk-margin-bottom">
						<acl-parse-error :errorData="errData">
							<button class="uk-button uk-button-primary" slot="further-action" @click="getVariations()">
								Retry
							</button>
						</acl-parse-error>
					</div>
					<div v-else>
						<template v-if="products && products.length > 0">
							<div class="uk-width-1-1 uk-flex uk-flex-column">
								<div class="tiny-padding" v-for="(product, key) in products" :key="product.uuid" :class="key < (products.length - 1) ? 'border-bottom' : ''">

									<div class="uk-flex uk-flex-middle grid-gutter uk-padding-remove-bottom click" @click.prevent="handleChangeEvent(product.uuid)">
										<div class="uk-light tiny-margin-right tiny-margin-top uk-width-auto uk-margin-right">
											<div class="square-25 uk-border-rounded uk-flex uk-flex-column uk-flex-center uk-flex-middle" :class="productSelection.includes(product.uuid) ? 'uk-background-primary' : 'bordered'">
												<span class="mdi mdi-check uk-text-white" v-if="productSelection.includes(product.uuid)"></span>
											</div>
										</div>
										...
										<div class="uk-width-small">
											<span class="text-small">
												{{product.variation.name | capitalize }}
											</span>
										</div>
									</div>
								</div>
								...
								<div class="uk-width-1-1 uk-flex uk-flex-column uk-flex-center uk-flex-middle">
									<div class="uk-flex uk-flex-column uk-flex-center uk-flex-middle">
										...
										<p class="uk-text-center uk-margin-remove-top">
											Product Variations will show up here for selection when available
										</p>

										<button class="uk-button uk-button-primary" @click="clearFilters();getVariations()">
											Reset All Filters
										</button>
		...
		<vue-final-modal v-model="loadModal" classes="uk-width-1-1 uk-flex uk-flex-center" content-class="uk-margin-auto uk-margin-auto-vertical" :esc-to-close="false" :click-to-close="false" overlay-class="uk-overlay-primary" @before-open="getData()" v-if="(productSelection && productSelection.length > 0) && to && from">
				...
				<div class="uk-width-1-1 background bordered uk-border-rounded raised-deep uk-animation-scale-up uk-animation-fast uk-width-5-6@xl uk-margin-auto " style="max-width:99vw;">
					...
					<div class="modal-content uk-overflow-auto uk-width-1-1">
						<div class="uk-width-1-1 uk-flex" :class="`uk-child-width-1-${stats.length+1}`">
							<div class="uk-width-expand border-right">
								...
								<div class="grid-padding uk-flex uk-flex-middle uk-flex-between">
									...
									<div class="label uk-border-rounded click bordered" :class="showTotals ? 'uk-background-danger' : 'uk-background-success'" @click="showTotals = !showTotals">
										<span class="text-small uk-text-white">
								...
								<div v-if="periods && periods.length > 0">
									<template v-for="(period, key) in periods">
										...
										<!-- <div class="tiny-padding uk-flex border-bottom" :class="key > periods.length ? 'border-top' : ''">
											<div class="chip uk-border-rounded bordered click" @click="togglePeriodVisibility(period)">
												{{$moment(period).format('DD MMMM, YYYY')}} - Click To {{ hiddenPeriods.includes(period) ? 'Show' : 'Hide' }} Period
											</div>
										</div> -->
						...
						<div v-for="(item, key) in stats" :key="key" :class="key < stats.length -1 ? 'border-right' : ''">
							...
							<div class="grid-padding uk-flex uk-flex-middle">
								<h6 class="uk-margin-remove">
									&nbsp;
								</h6>

								<div class="label bordered" style="border-color: transparent !important;">
									<span>&nbsp;</span>
								</div>
							</div>
...
<script>
import {products} from "@/mixins/reports/compare" 
import productVariations from "@/mixins/product/productVariations"
export default{
	head(){
		return {
			title : `${this.$appName} | ${this.emporium.name} - Compare Products`
		}
	},
	...
	mixins : [products, productVariations],
	...
	methods : {
		...
		handleChangeEvent(value){
			...
			if (!this.productSelection.includes(value)) {
				if (this.productSelection.length < 4) {
					this.productSelection.push(value)
					return
				}
				this.$message.error("You can only compare up to 4 products at a time.", 3)
				return
			}

			this.productSelection = this.productSelection.filter(i => {return value !== i })
		}
	}
};
</script>
