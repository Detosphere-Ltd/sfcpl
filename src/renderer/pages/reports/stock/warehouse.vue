	...
	<div class="background">
		<div class="tiny-padding border-bottom uk-flex uk-flex-between">
			<div class="uk-width-expand uk-flex uk-flex-middle">
				...
				<h4 class="uk-margin-remove">Warehouse Stock Report</h4>
			</div>
		...
		<section class="uk-width-1-1 uk-flex uk-flex-column horizontal-scroll">
			<headers-reports-stock class="tiny-padding border-bottom"></headers-reports-stock>
					...
					<div v-else>
						<template v-if="stats && stats.length > 0">
							<div class="uk-width-1-1 uk-flex uk-flex-column">
								...
								<stock-warehouse-report class="tiny-padding"  v-for="(item,i) in stats" :key="i" :item="item" :class="i < (stats.length - 1) ? 'border-bottom' : ''">
...
<script>
import {warehouse} from "@/mixins/reports/stock" 
export default{
	head(){
		return {
			title : `${this.$appName} | ${this.emporium.name} - Warehouse Stock Report`
		}
	},
	data(){
		return {
